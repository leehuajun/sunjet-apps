 ...
 */
public class DealerExpenseSummaryVM extends FlowListBaseVM {
...
//        this.setHeaderRows(2);   // 设置搜索栏的行数，默认是1行
    ...
    public void search() {
        if (this.getEndDate().getTime() <= this.getStartDate().getTime()) {
            ...
            maps.clear();
            String reportName;
            if ("所有类型总费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM total_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("三包费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM warranty_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("活动费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM activity_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("首保费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM first_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("故障件运费".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM freight_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("外出费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM out_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("配件费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM part_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            } else if ("工时费用".equals(this.getType())) {
                maps = jdbcTemplate.queryForList(
                        "SELECT * FROM working_expense_summary " +
                                "WHERE dealer_name LIKE  '%" + this.getDealerName() + "%' ");
            }
...
//        new SearchOrder("name", SearchOrder.OrderType.ASC, 2)
    ...
    public void exportExcel() {
        ...
        for (Map<String, Object> map : maps) {
            ...
            datarow.createCell(0).setCellValue(map.get("服务站") == null ? "" : map.get("agency_name").toString());
