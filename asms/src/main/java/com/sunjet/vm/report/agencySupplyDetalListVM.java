 ...
 */
public class agencySupplyDetalListVM extends FlowListBaseVM {
...
//        this.setHeaderRows(2);   // 设置搜索栏的行数，默认是1行
        maps = jdbcTemplate.queryForList(
                "SELECT * FROM agency_supply_summary " +
...
//        new SearchOrder("name", SearchOrder.OrderType.ASC, 2)
    ...
    public void exportExcel() {
        ...
        cell.setCellValue("合作商编号");
        cell.setCellStyle(style);
        cell = row.createCell(1);
        cell.setCellValue("合作商名称");
        cell.setCellStyle(style);
        ...
        for (Map<String, Object> map : maps) {
            ...
            datarow.createCell(0).setCellValue(map.get("agency_code") == null ? "" : map.get("agency_code").toString());
            datarow.createCell(1).setCellValue(map.get("agency_name") == null ? "" : map.get("agency_name").toString());
            datarow.createCell(2).setCellValue(map.get("src_doc_type") == null ? "" : map.get("src_doc_type").toString());
            datarow.createCell(3).setCellValue(map.get("doc_no") == null ? "" : map.get("doc_no").toString());
            datarow.createCell(4).setCellValue(map.get("part_expense") == null ? "" : df.format(map.get("part_expense")));
            datarow.createCell(5).setCellValue(map.get("transport_expense") == null ? "" : df.format(map.get("transport_expense")));
            Double sum = null;
            if (map.get("part_expense") != null && map.get("transport_expense") != null) {
                sum = (Double) map.get("part_expense") + (Double) map.get("transport_expense");
            }
            datarow.createCell(6).setCellValue(sum == null ? "" : df.format(sum));
            datarow.createCell(7).setCellValue(map.get("created_time") == null ? "" : map.get("created_time").toString());
