 ...
 */
public class RecycleReportVM extends FormBaseVM {
...
//        JSONArray jsonArray = JSONArray.fromObject(iconList);
        String data = Executions.getCurrent().getArg().get("objId").toString();
        ...
        if ("recycle".equals(type)) {
            this.reportUrl = "/views/report/asm/report.jsp?data=" + data + "&report=recycle.jasper";
        } else {
            this.reportUrl = "/views/report/asm/report.jsp?data=" + data + "&docNo=" + docNo + "&dealerCode=" + dealerCode + "&report=recycleReport.jasper";
        }
