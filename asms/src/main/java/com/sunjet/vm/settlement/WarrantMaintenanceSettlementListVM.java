 ...
 */
public class WarrantMaintenanceSettlementListVM extends FlowListBaseVM {
    @WireVariable
    ...
    public void init() {
        ...
        initList();
        this.setEnableDelete(hasPermission(DealerSettlementEntity.class.getSimpleName() + ":delete"));
        this.setEnableAdd(hasPermission(DealerSettlementEntity.class.getSimpleName() + ":create"));
        this.setEnableUpdate(hasPermission(DealerSettlementEntity.class.getSimpleName() + ":modify"));
...
//        new SearchOrder("name", SearchOrder.OrderType.ASC, 2)
    ...
    protected void configSpecification(SearchDTO searchDTO) {
        Specification<DealerSettlementEntity> specification = (root, query, cb) -> {
