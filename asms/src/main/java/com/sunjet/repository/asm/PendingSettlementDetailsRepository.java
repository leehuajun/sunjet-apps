...
public interface PendingSettlementDetailsRepository extends JpaRepository<PendingSettlementDetailsEntity, String>, JpaSpecificationExecutor<PendingSettlementDetailsEntity> {
    ...
    @Query("select fm from PendingSettlementDetailsEntity fm  where  fm.enabled=1 and fm.settlement=0 and fm.settlementDocType like '%配件结算单' and fm.agencyCode= ?1 and  fm.businessDate between ?2 and ?3")
    ...
    @Query("select fm from PendingSettlementDetailsEntity fm  where  fm.enabled=1 and fm.settlement=0 and fm.settlementDocType like '%服务结算单' and fm.dealerCode= ?1 and  fm.businessDate between ?2 and ?3")
    List<PendingSettlementDetailsEntity> getDealerSelttlements(String dealerCode, Date startDate, Date endDate);

    @Query("select fm from PendingSettlementDetailsEntity fm  where  fm.enabled=1 and fm.settlement=0 and fm.settlementDocType like '%运费结算单' and fm.dealerCode= ?1 and  fm.businessDate between ?2 and ?3")
