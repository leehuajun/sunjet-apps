...
public interface DealerRepository extends JpaRepository<DealerEntity, String>, JpaSpecificationExecutor<DealerEntity> {
    ...
    @Query("select de from DealerEntity de where de.enabled=1 and de.parent.objId=?1 and de.code like ?2")
    List<DealerEntity> findChildrenByParentIdAndFilter(String objId, String filter);

    @Query("select distinct de.parent from DealerEntity de where de.enabled=1 and de.code=?1")
    DealerEntity findParentDealerByDealerCode(String DealerCode);

    @Query("select de from DealerEntity de where de.enabled=1 and de.serviceManager.objId=?1 and (de.code like ?2 or de.name like ?2)")
