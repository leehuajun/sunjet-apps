...
public class CommonTreeUtil<T> {
    ...
    public CustomTreeNode getRoot(List<T> nodeList) {
        ...
        for (T t : nodeList) {
            TreeNodeEntity nodeEntity = (TreeNodeEntity) t;
...
//                }
    ...
    public Boolean isLeaf(T node, List<T> nodeList) {
        for (T t : nodeList) {
            if ((((TreeNodeEntity) t).getParent()) == ((TreeNodeEntity) node)) {
                return false;
            }
        }
        return true;
...
//    }
    private void getNode(CustomTreeNode<T> parent, List<T> nodeList) {
        for (T t : nodeList) {
            if (((TreeNodeEntity) t).getParent() == ((TreeNodeEntity) parent.getData())) {
